describe('Product Form Test', () => {
    ...
    it('Submit puting data in wrong format', () => {
        ...
        cy.get('#inputNumber-Price').type('A');
        ...
        cy.get('#inputNumber-Price').should('have.css', 'background-color', 'rgb(254, 202, 202)').parent().should('contain', 'Set a number');
        cy.get('#inputString-Description').should('have.css', 'background-color', 'rgb(254, 202, 202)').parent().should('contain', 'The size should be between 1 and 500');
        ...
        cy.get('#inputNumber-Price').clear().should('have.css', 'background-color', 'rgb(254, 202, 202)').parent().should('contain', 'Set a number');
        cy.get('#inputNumber-Price').type('1,3').should('have.css', 'background-color', 'rgb(249, 250, 251)').parent().should('not.contain', 'Set a number');

        cy.get('#inputString-Description').clear().should('have.css', 'background-color', 'rgb(254, 202, 202)').parent().should('contain', 'The size should be between 1 and 500');
        cy.get('#inputString-Description').type('correct state').should('have.css', 'background-color', 'rgb(249, 250, 251)').parent().should('not.contain', 'The size should be between 1 and 500');
    })
})
